...
export default function LikedVideos() {
  const { likedIds, videos } = useNotTube();
  const items = videos.filter((v) => likedIds.includes(String(v.id)));

  return (
    <ShellLayout active="liked">
      <style>{`
      ...
      <h2 style={{margin:"0 0 12px"}}>Liked videos</h2>
      {items.length === 0 ? (
        <div className="empty">No likes yet.</div>
      ) : (
        <div className="grid">
          {items.map(v=>(
